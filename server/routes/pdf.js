...
router.post('/', bodyParser, function (req, res, next) {
    ...
    fs.writeFile('./uploads/test/pdf.html', html, function (err, file) {
        ...
        pdf.create(html, options).toFile('./uploads/test/test.pdf', function (err, pdf) {
            if (err) return console.log(err);

            let file = './uploads/test/test.pdf';
            res.download(file, 'test.pdf'); // Set disposition and send it.
        });
    });
});
